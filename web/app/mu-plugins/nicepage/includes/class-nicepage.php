            ...
            function ($productItemMatch) {
                ...
                $productItemHtml = preg_replace_callback(
                    ...
                    function ($titleMatch) {
                        ...
                        if ($postUrl) {
                            ...
                            if (isset($postTitle) && $postTitle != '') {
                                $titleHtml = preg_replace('/<!--product_title_content-->([\s\S]+?)<!--\/product_title_content-->/', $postTitle, $titleHtml);
                            }
                        }
                        return $titleHtml;
                    },
                ...
                $productItemHtml = preg_replace_callback(
                    ...
                    function ($textMatch) {
                        ...
                        $postContent = plugin_trim_long_str(NpAdminActions::getTheExcerpt(Nicepage::$_post->ID), 150);
                        if (isset($postContent) && $postContent != '') {
                            $textHtml = preg_replace('/<!--product_content_content-->([\s\S]+?)<!--\/product_content_content-->/', $postContent, $textHtml);
                        }
                        return $textHtml;
                    },
                    $productItemHtml
                ...
                $productItemHtml = preg_replace_callback(
                    ...
                    function ($buttonMatch) {
                        ...
                        $product = wc_get_product(Nicepage::$_postId);
                        $buttonText = sprintf(__('%s', 'woocommerce'), $product->add_to_cart_text());
                        $buttonHtml = preg_replace('/href=[\'|"][\s\S]+?[\'|"]/', 'href="%s"', $buttonHtml);
                        $buttonHtml = preg_replace('/<!--product_button_content-->([\s\S]+?)<!--\/product_button_content-->/', '%s', $buttonHtml);
                        if ($goToProduct) {
                            $buttonHtml = sprintf(
                                $buttonHtml,
                            ...
                            $buttonHtml = preg_replace('/class=[\'|"]([\s\S]+?)[\'|"]/', 'class="$1 %s"', $buttonHtml);
                            $buttonHtml = str_replace('href', 'data-quantity="1" data-product_id="%s" data-product_sku="%s" href', $buttonHtml);
                            $buttonHtml = apply_filters(
                                'woocommerce_loop_add_to_cart_link',
                                sprintf(
                                    $buttonHtml,
                                    esc_attr(Nicepage::$_postId),
                ...
                $productItemHtml = preg_replace_callback(
                    ...
                    function ($priceHtml) {
                        ...
                        $priceHtml = preg_replace('/<!--product_old_price_content-->([\s\S]*?)<!--\/product_old_price_content-->/', $old_price, $priceHtml);
                        return preg_replace('/<!--product_regular_price_content-->([\s\S]+?)<!--\/product_regular_price_content-->/', get_woocommerce_currency_symbol() . $price, $priceHtml);
                    },
                    $productItemHtml
                );
...
SCRIPT;
                ...
                return $script . '<div class="widget_shopping_cart_content">' . $shoppingCartHtml . '</div>';
            },
            $content
        );
        return $content;
